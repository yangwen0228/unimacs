...
;; will be applied if you jump-prev (move up the tree). However, the diff display
...
;; ==============
;;
;; Emacs allows a very useful and powerful method of jump-preving only selected
;; changes: when a region is active, only changes that affect the text within
;; that region will be jump-prevne. With the standard Emacs jump-prev system, changes
;; produced by jump-preving-in-region naturally get added onto the end of the
;; linear jump-prev history:
;;
;;                       o
;;                       |
;;                       |  x  (second jump-prev-in-region)
;;                       o  |
;;                       |  |
;;                       |  o  (first jump-prev-in-region)
;;                       o  |
;;                       | /
;;                       |/
;;                       o
;;
;; You can of course jump-next these jump-prevs-in-region as usual, by jump-preving the
;; jump-prevs:
;;
;;                       o
;;                       |
...
;;                       o  | \
;;                       |  |  |
;;                       |  o  o  (jump-prev the jump-prev-in-region)
;;                       o  |  |
;;                       | /   |
;;                       |/    |
;;                       o     x  (jump-prev the jump-prev-in-region)
;;
;;
;; In `jump-tree-mode', jump-prev-in-region works much the same way: when there's
;; an active region, jump-preving only jump-preves changes that affect that region. In
;; `jump-tree-mode', jump-nexting when there's an active region similarly only
;; jump-nextes changes that affect that region.
;;
;; However, the way these jump-prev- and jump-next-in-region changes are recorded in the
;; jump-prev history is quite different. The good news is, you don't need to
;; understand this to use jump-prev- and jump-next-in-region in `jump-tree-mode' - just
...
;; the jump-prev tree as you jump-prev- and jump-next-in-region, then read on...
;;
;;
;; Jump-Prev-in-region creates a new branch in the jump-prev history. The new branch
;; consists of an jump-prev step that jump-preves some of the changes that affect the
;; current region, and another step that jump-preves the remaining changes needed
;; to rejoin the previous jump-prev history.
;;
;;      Previous jump-prev history                Jump-Prev-in-region
;;
;;               o                                o
;;               |                                |
;;               |                                |
...
;;               o                                o_
;;               |                                | \
;;               |                                |  x  (jump-prev-in-region)
;;               |                                |  |
;;               x                                o  o
;;
;; As long as you don't change the active region after jump-preving-in-region,
;; continuing to jump-prev-in-region extends the new branch, pulling more changes
;; that affect the current region into an jump-prev step immediately above your
;; current location in the jump-prev tree, and pushing the point at which the new
;; branch is attached further up the tree:
;;
;;      First jump-prev-in-region                 Second jump-prev-in-region
;;
;;               o                                o
;;               |                                |
;;               |                                |
...
;;       o  o                 o  o
;;
;; Jump-Nexting takes you back down the jump-prev tree, as usual (as long as you haven't
;; changed the active region after jump-preving-in-region, it doesn't matter if it
;; is still active):
;;
;;                       o
;;           |
...
;;
;;
;; What about jump-next-in-region? Obviously, jump-next-in-region only makes sense if
;; you have already jump-prevne some changes, so that there are some changes to
;; jump-next! Jump-Nexting-in-region splits off a new branch of the jump-prev history below
;; your current location in the jump-prev tree. This time, the new branch consists
;; of a first jump-next step that jump-nextes some of the jump-next changes that affect the
;; current region, followed by *all* the remaining jump-next changes.
;;
;;      Previous jump-prev history                Jump-Next-in-region
;;
;;               o                                o
;;               |                                |
;;               |                                |
...
;;               o                                o  o
;;
;; As long as you don't change the active region after jump-nexting-in-region,
;; continuing to jump-next-in-region extends the new branch, pulling more jump-next
;; changes into a jump-next step immediately below your current location in the
;; jump-prev tree.
;;
;;      First jump-next-in-region                 Second jump-next-in-region
;;
;;               o                                 o
;;               |                                 |
;;               |                                 |
...
;;               o  o                              o  o
;;
;; Note that jump-prev-in-region and jump-next-in-region only ever add new changes to
;; the jump-prev tree, they *never* modify existing jump-prev history. So you can always
...
(defvar buffer-jump-tree nil
  "Tree of jump-prev entries in current buffer.")
(put 'buffer-jump-tree 'permanent-local t)
(make-variable-buffer-local 'buffer-jump-tree)
...
(defun jump-tree-grow (jump-prev)
  "Add an JUMP-PREV node to current branch of `buffer-jump-tree'."
  (let* ((current (jump-tree-current buffer-jump-tree))
         (new (jump-tree-make-node current jump-prev)))
    (push new (jump-tree-node-next current))
    (setf (jump-tree-current buffer-jump-tree) new)))
...
that are already part of `buffer-jump-tree'."
...
(defmacro jump-tree-num-branches ()
  ...
  '(length (jump-tree-node-next (jump-tree-current buffer-jump-tree))))
...
(make-variable-buffer-local '*jump-tree-id-counter*)
...
(defun jump-tree-move-GC-elts-to-pool (elt)
  ;; Move elements that can be garbage-collected into `buffer-jump-tree'
...
(defun jump-prev-list-pop-changeset (&optional discard-pos)
  ;; Pop changeset from `buffer-jump-prev-list'. If DISCARD-POS is non-nil, discard
  ...
  (while (or (null (car buffer-jump-prev-list))
             (and discard-pos (integerp (car buffer-jump-prev-list))))
    (setq buffer-jump-prev-list (cdr buffer-jump-prev-list)))
  ;; pop elements up to next jump-prev boundary, discarding position entries if
  ;; DISCARD-POS is non-nil
  (if (eq (car buffer-jump-prev-list) 'jump-tree-canary)
      (push nil buffer-jump-prev-list)
    (let* ((changeset (list (pop buffer-jump-prev-list)))
           (p changeset))
      (while (progn
               (jump-tree-move-GC-elts-to-pool (car p))
               (while (and discard-pos (integerp (car buffer-jump-prev-list)))
                 (setq buffer-jump-prev-list (cdr buffer-jump-prev-list)))
               (and (car buffer-jump-prev-list)
                    (not (eq (car buffer-jump-prev-list) 'jump-tree-canary))))
        (setcdr p (list (pop buffer-jump-prev-list)))
        (setq p (cdr p)))
      changeset)))

(defun jump-tree-copy-list (jump-prev-list)
  ;; Return a deep copy of first changeset in `jump-prev-list'. Object id's are
  ;; replaced by corresponding objects from `buffer-jump-tree' object-pool.
  (let (copy p)
...
(defun jump-prev-list-transfer-to-tree ()
  ;; Transfer entries accumulated in `buffer-jump-prev-list' to `buffer-jump-tree'.

  ;; `jump-prev-list-transfer-to-tree' should never be called when jump-prev is disabled
  ;; (i.e. `buffer-jump-tree' is t)
  (assert (not (eq buffer-jump-tree t)))

  ;; if `buffer-jump-tree' is empty, create initial jump-tree
  (when (null buffer-jump-tree) (setq buffer-jump-tree (make-jump-tree)))
  ;; make sure there's a canary at end of `buffer-jump-prev-list'
  (when (null buffer-jump-prev-list)
    (setq buffer-jump-prev-list '(nil jump-tree-canary)))

  (unless (or (eq (cadr buffer-jump-prev-list) 'jump-tree-canary)
              (eq (car buffer-jump-prev-list) 'jump-tree-canary))
    ;; create new node from first changeset in `buffer-jump-prev-list', save old
    ;; `buffer-jump-tree' current node, and make new node the current node
    (let* ((node (jump-tree-make-node nil (jump-prev-list-pop-changeset)))
           (splice (jump-tree-current buffer-jump-tree))
           (size (jump-prev-list-byte-size (jump-tree-node-jump-prev node)))
           (count 1))
      (setf (jump-tree-current buffer-jump-tree) node)
      ;; grow tree fragment backwards using `buffer-jump-prev-list' changesets
      (while (and buffer-jump-prev-list
                  (not (eq (cadr buffer-jump-prev-list) 'jump-tree-canary)))
        (setq node
        ...
        (incf count))
      ;; if no jump-prev history has been discarded from `buffer-jump-prev-list' since
      ;; last transfer, splice new tree fragment onto end of old
      ;; `buffer-jump-tree' current node
      (if (or (eq (cadr buffer-jump-prev-list) 'jump-tree-canary)
              (eq (car buffer-jump-prev-list) 'jump-tree-canary))
          (progn
            (setf (jump-tree-node-previous node) splice)
            (push node (jump-tree-node-next splice))
            (setf (jump-tree-node-branch splice) 0)
            (incf (jump-tree-size buffer-jump-tree) size)
            (incf (jump-tree-count buffer-jump-tree) count))
        ;; if jump-prev history has been discarded, replace entire
        ;; `buffer-jump-tree' with new tree fragment
        (setq node (jump-tree-grow-backwards node nil))
        (setf (jump-tree-root buffer-jump-tree) node)
        (setq buffer-jump-prev-list '(nil jump-tree-canary))
        (setf (jump-tree-size buffer-jump-tree) size)
        (setf (jump-tree-count buffer-jump-tree) count)
        (setq buffer-jump-prev-list '(nil jump-tree-canary))))
...
(defun jump-prev-list-rebuild-from-tree ()
  "Rebuild `buffer-jump-prev-list' from information in `buffer-jump-tree'."
  (unless (eq buffer-jump-prev-list t)
    (jump-prev-list-transfer-to-tree)
    (setq buffer-jump-prev-list nil)
    (when buffer-jump-tree
      (let ((stack (list (list (jump-tree-root buffer-jump-tree)))))
        ...
        (while (or (car stack)
                   (not (eq (car (nth 1 stack))
                            (jump-tree-current buffer-jump-tree))))
          (if (car stack)
              (progn
                (setq buffer-jump-prev-list
                      (append (jump-tree-node-jump-prev (caar stack))
                              buffer-jump-prev-list))
                (jump-prev-boundary)
...
(defun jump-tree-discard-node (node)
  ;; Discard NODE from `buffer-jump-tree', and return next in line for
  ;; discarding.

  ;; don't discard current node
  (unless (eq node (jump-tree-current buffer-jump-tree))

    ;; discarding root node...
    (if (eq node (jump-tree-root buffer-jump-tree))
        (cond
 ...
 has multiple branches"))
      ...
      (let* ((parent (jump-tree-node-previous node))
        ...
        (if (or (eq parent (jump-tree-current buffer-jump-tree))
                (and (jump-tree-node-next parent)
                     (or (not (eq parent (jump-tree-root buffer-jump-tree)))
...
set by `jump-prev-limit', `jump-prev-strong-limit' and `jump-prev-outer-limit'."
  ...
  (when (> (jump-tree-size buffer-jump-tree) jump-prev-limit)
    ...
    (let ((node (if (> (length (jump-tree-node-next
                                (jump-tree-root buffer-jump-tree))) 1)
                    (jump-tree-oldest-leaf (jump-tree-root buffer-jump-tree))
                  (jump-tree-root buffer-jump-tree))))

      ;; discard nodes until memory use is within `jump-prev-strong-limit'
      (while (and node
                  (> (jump-tree-size buffer-jump-tree) jump-prev-strong-limit))
      ...
      (while (and node
                  ...
                  (> (jump-tree-size buffer-jump-tree) jump-prev-limit)
                  (> (- (jump-tree-size buffer-jump-tree)
      ...
      (when (> (jump-tree-size buffer-jump-tree) jump-prev-outer-limit)
        ...
        (if jump-prev-ask-before-discard
            (when (yes-or-no-p
                   (format
                    ...
                    (buffer-name) (jump-tree-size buffer-jump-tree)))
              (setq buffer-jump-tree nil))
...
(defun jump-tree-pull-jump-prev-in-region-branch (start end)
  ...
  (if (jump-tree-reverting-jump-next-in-region-p start end)
    ...
    (let* ((region-changeset (list nil))
           ...
           (delta-list (list nil))
           (d delta-list)
           (node (jump-tree-current buffer-jump-tree))
      ...
      (if (null region-changeset)
          (when original-current
            ...
            (let ((mark-active nil))
              (while (not (eq (jump-tree-current buffer-jump-tree)
                              original-current))
        ...
        ;; started
        (let ((mark-active nil)
              (current (jump-tree-current buffer-jump-tree)))
          (while (not (eq (jump-tree-current buffer-jump-tree) node))
            (jump-tree-jump-prev-1))
          (while (not (eq (jump-tree-current buffer-jump-tree) current))
            (jump-tree-jump-next-1)))
        (cond
         ...
         ((null fragment)
          ...
          (setf (jump-tree-current buffer-jump-tree) fragment))
         ...
         (t
          ...
          (when repeated-jump-prev-in-region
            (setf (jump-tree-current buffer-jump-tree)
                  (jump-tree-node-previous original-fragment))
            (let ((mark-active nil))
              (while (not (eq (jump-tree-current buffer-jump-tree) splice))
        ...
        (while (progn
                 (and (setq node (car (jump-tree-node-next node)))
                      ...
                      (incf (jump-tree-count buffer-jump-tree))
                      (incf (jump-tree-size buffer-jump-tree)
                            (+ (jump-prev-list-byte-size (jump-tree-node-jump-prev node))
                               (jump-prev-list-byte-size (jump-tree-node-jump-next node)))))))
...
(defun jump-tree-pull-jump-next-in-region-branch (start end)
  ...
  (if (jump-tree-reverting-jump-prev-in-region-p start end)
    ...
    (let* ((region-changeset (list nil))
      ...
      ;; --- integrate branch into tree ---
      (setq node (jump-tree-current buffer-jump-tree))
...
(defun jump-tree-repeated-jump-prev-in-region-p (start end)
  ...
  ;; jump-prev-in-region
  (let ((node (jump-tree-current buffer-jump-tree)))
    (and (setq node
...
Within the jump-tree visualizer, the following keys are available:
  ...
  ;; if disabling `jump-tree-mode', rebuild `buffer-jump-prev-list' from tree so
...
changes within the current region."
  ...
  (when (eq buffer-jump-prev-list t)
...
(defun jump-tree-jump-prev-1 (&optional arg preserve-jump-next preserve-timestamps)
  ;; Internal jump-prev function. An active mark in `transient-mark-mode', or
  ;; non-nil ARG otherwise, enables jump-prev-in-region. Non-nil PRESERVE-JUMP-NEXT
  ...
  (let ((jump-prev-in-progress t)
        ...
        pos current)
    ;; transfer entries accumulated in `buffer-jump-prev-list' to
    ;; `buffer-jump-tree'
    (jump-prev-list-transfer-to-tree)
    (dotimes (i (or (and (numberp arg) (prefix-numeric-value arg)) 1))
      ;; check if at top of jump-prev tree
      (unless (jump-tree-node-previous (jump-tree-current buffer-jump-tree))
      ...
      (when (and jump-prev-in-region
                 (not (jump-tree-pull-jump-prev-in-region-branch
                       (region-beginning) (region-end))))
        (user-error "No further jump-prev information for region"))
      ;; remove any GC'd elements from node's jump-prev list
      (setq current (jump-tree-current buffer-jump-tree))
      (decf (jump-tree-size buffer-jump-tree)
      ...
      (incf (jump-tree-size buffer-jump-tree)
            (jump-prev-list-byte-size (jump-tree-node-jump-prev current)))
      ;; jump-prev one record from jump-prev tree
      (when jump-prev-in-region
        (setq pos (set-marker (make-marker) (point)))
        (set-marker-insertion-type pos t))
      (primitive-jump-prev 1 (jump-tree-copy-list (jump-tree-node-jump-prev current)))
      (jump-prev-boundary)
      ;; if preserving old jump-next record, discard new jump-next entries that
      ;; `primitive-jump-prev' has added to `buffer-jump-prev-list', and remove any GC'd
      ...
      (if preserve-jump-next
        ...
        ;; `buffer-jump-prev-list' in current node's jump-next record, replacing
      ...
      (setf (jump-tree-current buffer-jump-tree)
            (jump-tree-node-previous (jump-tree-current buffer-jump-tree)))
      (unless preserve-timestamps
        (setf (jump-tree-node-timestamp (jump-tree-current buffer-jump-tree))
              (current-time)))
      ;; if jump-preving-in-region, record current node, region and direction so we
      ;; can tell if jump-prev-in-region is repeated, and re-activate mark if in
      ;; `transient-mark-mode'; if not, erase any leftover data
      (if (not jump-prev-in-region)
        ...
        (setf (jump-tree-node-jump-prev-beginning current) (region-beginning)
              (jump-tree-node-jump-prev-end current) (region-end))
        (set-marker pos nil)))
    ;; jump-prev deactivates mark unless jump-preving-in-region
    (setq deactivate-mark (not jump-prev-in-region))))
...
(defun jump-tree-jump-next-1 (&optional arg preserve-jump-prev preserve-timestamps)
  ...
  (let ((jump-prev-in-progress t)
    ...
    (dotimes (i (or (and (numberp arg) (prefix-numeric-value arg)) 1))
      ...
      (when (null (jump-tree-node-next (jump-tree-current buffer-jump-tree)))
        (user-error "No further jump-next information"))
      ;; if region is active, or a non-numeric prefix argument was supplied,
      ;; try to pull out a new branch of changes affecting the region
      (when (and jump-next-in-region
                 (not (jump-tree-pull-jump-next-in-region-branch
                       (region-beginning) (region-end))))
        (user-error "No further jump-next information for region"))
      ...
      (incf (jump-tree-size buffer-jump-tree)
            (jump-prev-list-byte-size (jump-tree-node-jump-next current)))
      ;; jump-next one record from jump-prev tree
      (when jump-next-in-region
        (setq pos (set-marker (make-marker) (point)))
        (set-marker-insertion-type pos t))
      (primitive-jump-prev 1 (jump-tree-copy-list (jump-tree-node-jump-next current)))
      (jump-prev-boundary)
      ;; advance current node in tree
      (setf (jump-tree-current buffer-jump-tree) current)
      ...
      (if preserve-jump-prev
        ...
        ;; `buffer-jump-prev-list' in current node's jump-prev record, replacing
        ;; existing entry if one already exists
        (decf (jump-tree-size buffer-jump-tree)
              (jump-prev-list-byte-size (jump-tree-node-jump-prev current)))
        (setf (jump-tree-node-jump-prev current)
      ...
      ;; if jump-nexting-in-region, record current node, region and direction so we
      ;; can tell if jump-next-in-region is repeated, and re-activate mark if in
      ;; `transient-mark-mode'
      (if (not jump-next-in-region)
          (jump-tree-node-clear-region-data current)
        (goto-char pos)
        (setf (jump-tree-node-jump-next-beginning current) (region-beginning)
              (jump-tree-node-jump-next-end current) (region-end))
        (set-marker pos nil)))
    ;; jump-next deactivates the mark unless jump-nexting-in-region
    (setq deactivate-mark (not jump-next-in-region))))
...
using `jump-tree-jump-next'."
  (interactive (list (or (and prefix-arg (prefix-numeric-value prefix-arg))
                         (and (not (eq buffer-jump-prev-list t))
  ...
  (jump-prev-list-transfer-to-tree)
  ;; switch branch
  (setf (jump-tree-node-branch (jump-tree-current buffer-jump-tree))
        branch)
...
(defun jump-tree-set (node &optional preserve-timestamps)
  ...
  (let ((path (make-hash-table :test 'eq))
        (n node))
    (puthash (jump-tree-root buffer-jump-tree) t path)
    ...
    ;; selected node
    (setq n (jump-tree-current buffer-jump-tree))
    (while (not (gethash n path))
      (setq n (jump-tree-node-previous n)))
    ;; ascend tree until intersection node
    (while (not (eq (jump-tree-current buffer-jump-tree) n))
  ...
  (set-register
   register (registerv-make
             (jump-tree-make-register-data
              (current-buffer) (jump-tree-current buffer-jump-tree))
             :print-func 'jump-tree-register-data-print-func))
  ;; record REGISTER in current node, for visualizer
  (setf (jump-tree-node-register (jump-tree-current buffer-jump-tree))
        register))
...
(defun jump-tree-visualize ()
  ...
  (let ((jump-tree buffer-jump-tree)
        (buff (current-buffer))
    ...
    (setq buffer-jump-tree jump-tree)
...
(defun jump-tree-visualize-jump-prev (&optional arg)
  ...
  (unless (eq major-mode 'jump-tree-visualizer-mode)
    (user-error "jump-tree mode not enabled in buffer"))
  (let ((old (jump-tree-current buffer-jump-tree))
        current)
...
(defun jump-tree-visualize-jump-next (&optional arg)
  ...
  (let ((old (jump-tree-current buffer-jump-tree))
    ...
    (let ((jump-tree-insert-face 'jump-tree-visualizer-active-branch-face)
          (inhibit-read-only t))
      (jump-tree-draw-node (jump-tree-current buffer-jump-tree)))
...
using `jump-tree-jump-next' or `jump-tree-visualizer-jump-next'."
  ...
  (goto-char (jump-tree-node-marker (jump-tree-current buffer-jump-tree)))
  (let ((jump-tree-insert-face 'jump-tree-visualizer-default-face)
        (inhibit-read-only t))
    (jump-tree-highlight-active-branch (jump-tree-current buffer-jump-tree)))
  ;; increment branch
  (let ((branch (jump-tree-node-branch (jump-tree-current buffer-jump-tree))))
    (setf (jump-tree-node-branch (jump-tree-current buffer-jump-tree))
          (cond
    ...
    (let ((inhibit-read-only t))
      ...
      (jump-tree-draw-node (jump-tree-current buffer-jump-tree) 'current))))
...
(defun jump-tree-visualizer-quit ()
  ...
  (unless (eq major-mode 'jump-tree-visualizer-mode)
    (user-error "jump-tree mode not enabled in buffer"))
  (jump-tree-clear-visualizer-data buffer-jump-tree)
...
at POS, or point if POS is nil."
  ...
  (let ((node (get-text-property pos 'jump-tree-node)))
    (when node
      ...
      (let ((inhibit-read-only t)) (jump-tree-draw-tree buffer-jump-tree))
      (when jump-tree-visualizer-diff (jump-tree-visualizer-update-diff)))))
  ...
  (if (or (and (numberp arg) (< arg 0)) (eq arg '-))
    ...
    (unwind-protect
      ...
      (jump-tree-expand-down
       (nth (jump-tree-node-branch (jump-tree-current buffer-jump-tree))
            (jump-tree-node-next (jump-tree-current buffer-jump-tree)))))
