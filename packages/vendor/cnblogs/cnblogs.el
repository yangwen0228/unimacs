...
(defun cnblogs-load-variables ()
  "加载各变量的值"
  (cnblogs-load-entry-list)             ;加载博文项列表
  (cnblogs-load-category-list)          ;加载博文分类
  (cnblogs-init-posts-in-category))

(defun cnblogs-init-posts-in-category ()
  (dolist (categorie cnblogs-category-list)
    (push (cons categorie nil) cnblogs-posts-in-category) ;先将该分类加入
...
(defun cnblogs-check-legal-for-delete (src-file)
  ...
  (and
   ...
   (if (equal (cnblogs-get-src-file-state (buffer-file-name))
       ...
       t
     (progn
       (message "This post has not been published, so you can't delete it!")
       nil))))
...
(defun cnblogs-fetch-field-old (field)
  (let* ((regexp
          (concat "^[ \t]*[#]+[\\+]?"
                  field
                  ":[^\n]*"))
