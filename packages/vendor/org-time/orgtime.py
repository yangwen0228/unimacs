...
def datetimeToday():
    """
    Use this function to get datetime object of current date,
...
class OrgLine(object):
    ...
    def getLevel(self,):
        """
        """
        levelPattern = re.compile(r'\*+')
        match = levelPattern.match(self.line)
        if match:
    ...
    def getClockTime(self,):
        ...
        if clock_pattern.match(line) is not None:
            ...
            if timeStrGroup is not None:
                ...
                if len(start_end) == 2:
                    return (self._parseTimeStr(start_end[0]),
                            self._parseTimeStr(start_end[1]), time)
        return (None, None, 0)
...
class TimeFrame_CurWeek(object):
    ...
    def __init__(self, ):
        """
        """
        curDateISOCanendar = datetimeToday().isocalendar()
        self._curWeek = curDateISOCanendar[1]
        self._curYear = curDateISOCanendar[0]

    def inTimeFrame(self, timeFrameDate):
        timeFrameDateCalendar = timeFrameDate.isocalendar()
        return (timeFrameDateCalendar[1] == self._curWeek) \
            and (timeFrameDateCalendar[0] == self._curYear)
...
class TimeFrame_CurMonth(object):
    ...
    def __init__(self, ):
        """
        """
        curDay = datetimeToday()
        self._curMonth = curDay.month
        self._curYear = curDay.year

    def inTimeFrame(self, timeFrameDate):
        return (self._curYear == timeFrameDate.year) \
            and (self._curMonth == timeFrameDate.month)
...
class TimeData(object):
    ...
    def addTime(self, tag, clockTime):
        """
        """
        startTime = clockTime[0]
        timeSpent = clockTime[2]
    ...
    def pieChartPath(self,):
        fileName = self._timeFrame.getTimeFrameName().\
                   replace(' ', '_')+"_"+self._orgFileName+".png"
...
# I only want to know the time spend for 1st level of task,
# do not need to know too detail information
if __name__ == '__main__':
    ...
    maxTag = max(timeDataToday.getTagCount(), timeDataCurWeek.getTagCount(),
                 timeDataCurMonth.getTagCount())

    orgTable = orgtimetxt.OrgTable(2)
    ...
    for timeData in [timeDataToday, timeDataCurWeek, timeDataCurMonth]:
        if timeData.totalTime() is not 0:
            ...
            print(orgtimetxt.fileLink(timeData.pieChartPath()) + "    ", end="")
